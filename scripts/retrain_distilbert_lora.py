"""Retrain DistilBERT+LoRA on enhanced dataset with correct import order."""
import os
import sys
import json
import time
...
# Now safe to import torch, transformers, peft
import torch
...
from src.config import (
    ...
    BERT_MAX_LEN, LORA_BATCH_SIZE, LORA_LR, LORA_EPOCHS,
    LORA_PATIENCE, LORA_MIN_DELTA, BERT_MODEL_NAME, BERT_WEIGHT_DECAY,
    BERT_WARMUP_RATIO, LORA_R, LORA_ALPHA, LORA_DROPOUT, LORA_TARGET_MODULES,
...
print("=" * 60)
print("Training DistilBERT + LoRA (Enhanced Dataset)")
print("=" * 60)
...
history_path = os.path.join(REPORTS_DIR, "distilbert_lora_history.json")
...
results = evaluate_classifier(wrapped, test_df["processed_text"].tolist(),
                              test_df["label"].tolist(), model_name="DistilBERT+LoRA")
...
results["trainable_parameters"] = int(trainable_params)
...
print(f"\nDistilBERT+LoRA retraining complete!")
