"""
Fine-tune flan-t5-small on CORRECTED de-escalatory reply data.
Reply is directed AT THE AGGRESSOR, not comforting the victim.
Uses strong prompt engineering.
"""
import os
import sys
...
print("=" * 60)
print("Fine-tuning FLAN-T5-small for De-escalatory Reply Generation")
print("=" * 60)

# Load corrected synthetic data
df = pd.read_csv("data/synthetic/reply_synthetic_v2.csv")
print(f"Loaded {len(df)} corrected synthetic pairs")
...
# The model is trained to take an instruction + hate speech and output de-escalation
INSTRUCTION = "respond to this hate speech with a calm, respectful de-escalation: "

def preprocess(data):
    inputs = [INSTRUCTION + text for text in data["cyberbullying_text"].tolist()]
    targets = data["de_escalatory_reply"].tolist()
...
# Training args - optimized for small dataset
...
# Quick validation on diverse examples
print("\nQuick validation:")
test_texts = [
...
print("FLAN-T5-small de-escalation training complete!")
