"""Test SmolLM2-135M-Instruct with 4-bit quantization."""
...
print("=" * 70)
print("SmolLM2-135M-Instruct (4-bit) Simplified Prompt Test")
print("=" * 70)
...
print("="*70)
cache_path = os.path.expanduser("~/.cache/huggingface/hub/models--huggingfacetb--sollm2-135m-instruct")
if os.path.exists(cache_path):
    total_size = sum(os.path.getsize(f) for f in glob.glob(cache_path + "/**/*", recursive=True) if os.path.isfile(f))
    print(f"SmolLM2-135M cache size: {total_size / (1024*1024):.2f} MB")
else:
    print("Cache path not found yet")

out_path = os.path.join("reports", "smollm2_135m_4bit_simple_prompt_10_examples.json")
