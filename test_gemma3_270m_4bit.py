"""Test Gemma 3 270M with 4-bit quantization."""
...
print("=" * 70)
print("Gemma 3 270M (4-bit) Simplified Prompt Test")
print("=" * 70)
...
print("="*70)
cache_path = os.path.expanduser("~/.cache/huggingface/hub/models--google--gemma-3-270m-it")
if os.path.exists(cache_path):
    total_size = sum(os.path.getsize(f) for f in glob.glob(cache_path + "/**/*", recursive=True) if os.path.isfile(f))
    print(f"Gemma 3 270M cache size: {total_size / (1024*1024):.2f} MB")
else:
    print("Cache path not found yet")

out_path = os.path.join("reports", "gemma3_270m_4bit_simple_prompt_10_examples.json")
